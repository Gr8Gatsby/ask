...
| Date | Change |
|---|---|
| 2026-05-16 | Mac v1.4.2: consolidate to a single Sessions surface. Remove the Settings → Sessions tab (local-only debug); keep the main-window Sessions tab as the only one. Fix `UnifiedSession.isActive` to trust the daemon (any non-terminal state is active) — previous `last_seen <= 10 min` filter was hiding long-running `running_tool` sessions whose last hook predated the window. ~200 lines of dead code removed. |
| 2026-05-16 | Mac v1.4.1: Sessions diagnostics. New **Session Health** Diagnostics section with per-session detail rows, live `ps` + tmux pane counts, and a `registered=N · live_processes=M · tmux_panes=K · missing=…` delta line. CloudKit section gains a per-record-type schema-presence probe. New Sessions-tab-readout section. Suppresses the misleading "Remote sessions unavailable" banner on accounts that have never written an `AskTask`. |
| 2026-05-16 | Mac v1.4.0: new **Sessions** tab in the Ask main window — unified active-sessions view merging local daemon registries with CloudKit `AskTask` records across every Mac signed in to the same iCloud account; 10-min active window, 30 s remote poll while visible, health dots (healthy / warning / errored / stalled), grouped by machine with "This Mac" first. Includes prior fixes (reliable hook delivery, PID-file lock-holder detection) and the terminal-mirror experiment behind `ASK_MIRROR=1`. |
...
| 2026-04-06 | v0.7.7: scripts zip for customer machine onboarding, diagnostics block (hook status, socket, version, logs), Collect Logs zip extraction, robust test coverage |
