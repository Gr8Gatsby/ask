...
| 2026-04-12 | Fix _get_process_cwd in transport.py: replace incorrect lsof -d cwd -Fn flags (which matched fd names "c","w","d") with plain lsof -p {pid} and parse the line where FD column == "cwd"; this caused CWD to always be empty, preventing CWD-based session deduplication and causing a spurious pid-XXXXX transient session |
| 2026-04-12 | Fix missing assistant messages in task feed: session_stop.py read last_assistant_message field which doesn't exist in Stop hook payload; pre_tool_use.py never captured Claude's message at all. Both hooks now read the session transcript JSONL (~/.claude/projects/{encoded-cwd}/{session_id}.jsonl) and extract the last assistant text block before sending to the daemon |
...
| 2026-05-06 | Fix tmux Claude messaging round-trip (v0.4.6): (1) `_tm_send_text` now uses `target=tmux_target` for tmux-managed sessions instead of always passing `session_id`, mirroring `_tm_send_interrupt`; previously terminal-manager rejected sends with "Unknown session" when the session wasn't pre-registered. (2) `_handle_session_start` now calls `_tm_register` BEFORE the A2A writes (`_set_task_status`, `_append_structured_message`); these were awaited synchronously and could stall under MCP load (e.g. supervisor session firing pre_tool_use storms), leaving new sessions unregistered with terminal-manager. A2A writes are now fire-and-forget. (3) `_route_text` `wait_for_pattern` regex relaxed from `[>?❯]\s*$` (end-of-line anchor) to `^[>❯]` (start-of-line) so it matches even when Claude's input box has hint/in-flight text after the prompt char; timeout reduced 30s→10s. (4) Daemon startup no longer blocks the event loop on `find_repos()` (which walks `~/Documents/code` and could hang indefinitely on TCC restrictions or slow filesystems); `find_repos` now runs in a thread with a 3 s timeout, and `_emit_start_session_block` + `_discover_active_processes` are spawned as background tasks so the socket server stays responsive while they complete. Verified end-to-end: routing 1 s, tmux receive ✓, hook back 1 s on a fresh tmux Claude. |
| 2026-05-12 | Defer `find_repos()` until user opts in (v0.4.7): the daemon no longer walks `~/Documents/code` on startup, which was firing a macOS Documents-folder TCC prompt on every launch. The start-session block emits with an empty repo list plus a `needs_scan` flag and a `scan_action` payload; clicking it sends `__scan_repos__` to the daemon which then calls `find_repos`, emits the populated picker, and caches the result at `~/.ask/claude3-repos.json`. Subsequent launches reuse the cached list and offer a `rescan_action` for when the user has added a new repo. Net effect: zero Documents prompts until the user explicitly opens the repo picker. |
