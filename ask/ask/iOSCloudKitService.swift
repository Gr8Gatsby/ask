...
final class iOSCloudKitService {
    ...
    /// Fetches all messages for a machine, sorted oldest first.
    func fetchMessages(machineID: String) async throws -> [AskMessage] {
        ...
        return results.compactMap { _, result in
            guard let record = try? result.get() else { return nil }
            return AskMessage(record: record)
        }
    }
    ...
    func updateTyping(machineID: String) async throws {
        ...
        if let existing = try? await database.record(for: recordID) {
            ...
            record = CKRecord(recordType: CKSchema.RecordType.typing, recordID: recordID)
        }
        record[CKSchema.Typing.machineID] = machineID
        record[CKSchema.Typing.fromDevice] = "iphone"
        record[CKSchema.Typing.timestamp] = Date()
        _ = try? await database.modifyRecords(saving: [record], deleting: [], savePolicy: .allKeys)
    }

    /// Deletes the iPhone typing indicator so the Mac sees dots disappear immediately.
    func clearTyping(machineID: String) async {
        let recordID = CKRecord.ID(recordName: "typing-\(machineID)-iphone")
        _ = try? await database.deleteRecord(withID: recordID)
    }

    /// Returns the Mac's most recent typing timestamp, or nil if none/expired.
    func fetchMacTypingTimestamp(machineID: String) async -> Date? {
        let recordName = "typing-\(machineID)-mac"
        let recordID = CKRecord.ID(recordName: recordName)
    ...
    func sendMessage(machineID: String, text: String, messageID: String) async throws {
