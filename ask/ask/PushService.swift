...
final class PushService: NSObject {
    ...
    private func saveSubscriptionsIfNeeded() async {
        let subID = "ask-rkblock-actionable-v1"
        // Legacy subscription IDs to clean up
        let legacyIDs = ["ask-rkblock-created", "ask-action-required-v1", "ask-rkblock-changes-v2"]
        ...
        // Silent push subscription — fires only for actionable RKBlocks (requiresResponse == 1).
        // Non-actionable blocks (status updates, feed items) are caught by the 15s poll.
