...
def is_session_emit(m: dict) -> bool:
    return (m.get('method') == 'tools/call' and
            ...
            m.get('params', {}).get('arguments', {}).get('blockType') == 'claude_session')
...
def test_tool_executed_emits_session():
    print('\nTest 2: tool_executed → claude_session block emitted')
    h = TestHarness()
    h.start()
    ...
    if call:
        ok('claude_session block emitted after tool_executed')
        ...
        fail('claude_session block emitted after tool_executed', 'timed out')
...
def test_session_stop():
    ...
    call = h.wait_for(is_session_emit, timeout=3.0)
    if call:
        ok('claude_session block emitted on session_stop')
        ...
        fail('claude_session block emitted on session_stop', 'timed out')
