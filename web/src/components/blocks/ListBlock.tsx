...
export default function ListBlock({ block, payload, onRespond }: Props) {
  return (
    <div className="flex flex-col gap-2">
      {payload.title && <p className="text-sm font-semibold text-white">{payload.title}</p>}
      <div className="border-t border-ask-sep">
        {payload.items.map(item => (
          <button
            key={item.id}
            onClick={() => onRespond(block.blockID, item.id)}
            className="w-full flex items-center justify-between py-2.5 border-b border-ask-sep last:border-0 hover:bg-white/5 transition-colors text-left px-0.5"
          >
            <div>
        ...
        <div className="flex flex-col gap-2 pt-1">
          {payload.actions.map(action => (
            <button
              ...
              className="w-full py-2 rounded-lg bg-ask-card2 text-sm font-medium text-white hover:bg-ask-card2/60 transition-colors"
            >
              {action}
            </button>
          ))}
